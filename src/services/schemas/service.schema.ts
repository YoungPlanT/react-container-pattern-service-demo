import { z } from "zod";
import createStringField from "../utils/schema.helper";
...
export const HeaderSectionSchema = z.object({
  ...
  title_emphasized_part: createStringField('title_base_part', {
    max: validationConfig.stringValidation.STRING_MAX_LENGTH,
  }).optional(),
  title_base_part: createStringField('title_base_part', {
    required: true,
    min: validationConfig.stringValidation.STRING_MIN_LENGTH,
    max: validationConfig.stringValidation.STRING_MAX_LENGTH,
  }),
...
export const CardSectionSchema = z.object({
  ...
  cards_data: z.array(z.string())
    .superRefine((items, ctx) => {
      if (items.length < validationConfig.arrayValidation.ARRAY_MIN_LENGTH) {
        ctx.addIssue({
      ...
      if (items.length > validationConfig.arrayValidation.ARRAY_MAX_LENGTH) {
        ctx.addIssue({
          code: z.ZodIssueCode.too_big,
          maximum: validationConfig.arrayValidation.ARRAY_MAX_LENGTH,
          type: 'string',
          origin: 'array',
          message: `Array 'cards_data' cannot contain more than ${validationConfig.arrayValidation.ARRAY_MAX_LENGTH} items`,
          params: {
...
export const NumberedListSectionSchema = z.object({
  ...
  numbered_list_data: z.array(z.string())
    .refine(
      (arr) => arr.length > validationConfig.arrayValidation.ARRAY_MIN_LENGTH,
      {
        message: "Array 'numbered_list_data' must contain at least one item",
        params: {
          errorCode: 'ARRAY_TOO_SMALL',
          min: validationConfig.arrayValidation.ARRAY_MIN_LENGTH,
          severity: 'error',
        }
      }
    )
    .refine(
      (arr) => arr.length <= validationConfig.arrayValidation.ARRAY_MAX_LENGTH,
      {
        message: `Array 'numbered_list_data' cannot contain more than ${validationConfig.arrayValidation.ARRAY_MAX_LENGTH} items`,
        params: {
          errorCode: 'ARRY_TOO_BIG',
          max: validationConfig.arrayValidation.ARRAY_MAX_LENGTH,
          severity: 'error',
        }
      }
    )
